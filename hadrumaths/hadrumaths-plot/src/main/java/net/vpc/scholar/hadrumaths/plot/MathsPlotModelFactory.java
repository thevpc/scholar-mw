...
import net.vpc.scholar.hadruplot.model.value.PlotValueFileType;
import net.vpc.scholar.hadruplot.model.value.PlotValueType;
import net.vpc.scholar.hadruplot.model.value.PlotValueArrayType;
...
//            container.add(_plotArr(other.toArray(new Expr[0])));
    ...
    private ExpressionsPlotModel _createExpressionsPlotModel(PlotBuilder builder) {
        ExpressionsPlotModel mm = (ExpressionsPlotModel) new ExpressionsPlotModel().setDomain(
                PlotConfigManager.getPlotDomainResolvers().resolve((Domain) builder.getDomain())
        )
