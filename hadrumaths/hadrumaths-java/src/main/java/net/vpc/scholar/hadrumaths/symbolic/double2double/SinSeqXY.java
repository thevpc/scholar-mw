...
//                fy = cos((n * PI / domain.Height) * (Y - domain.Ymin)); //ok
                fy = Maths.cos(Maths.mul(n, pibyh, Maths.sub(Y, ymin))); //ok
            } else {
//                fy = cos(((2 * n + 1) / 2 * PI / domain.Height) * (Y - domain.Ymin)); //ok
                fy = Maths.sin(Maths.mul(Maths.div(Maths.sum(Maths.mul(two, n), one), two), pibyh, Maths.sub(Y, ymin))); //ok
            }
        } else {
            if (maxSouth) {
                //fy = sin(((2 * n + 1) / 2 * PI / domain.Height) * (Y - domain.Ymin)); //ok
                fy = Maths.cos(Maths.mul(Maths.div(Maths.sum(Maths.mul(two, n), one), two), pibyh, Maths.sub(Y, ymin))); //ok
            } else {
                //fy = sin(((n+1) * PI / domain.Height) * (Y - domain.Ymin)); //ok
                fy = Maths.sin(Maths.mul(Maths.sum(n, one), pibyh, Maths.sub(Y, ymin))); //ok
            }
        }
        exprVal = (Maths.mul(fx, fy, Maths.expr(domain))).toDD();
    }
