    ...
    private void init(boolean maxNorth, boolean maxEast, boolean maxSouth, boolean maxWest, DoubleParam m, DoubleParam n, Domain domain) {
        ...
        if (maxWest) {
            if (maxEast) {
                ...
                fx = cos(MathsBase.mul(two, m, pibyw, MathsBase.sub(X, xmin))); //ok
            } else {
                //cos((m * 2+1)/2 * pi / width * (X-min))
                fx = cos(MathsBase.mul(MathsBase.div(MathsBase.sum(MathsBase.mul(two, m), one), two), pibyw, MathsBase.sub(X, xmin))); //ok
            }
        } else {
            if (maxEast) {
                //fx = sin(((2 * m + 1) / 2 * PI / domain.width) * (X - domain.xmin)); //ok
                fx = sin(MathsBase.mul(MathsBase.div(MathsBase.sum(MathsBase.mul(two, m), one), two), pibyw, MathsBase.sub(X, xmin))); //ok
            } else {
//                fx = sin(((m+1) * PI / domain.width) * (X - domain.xmin)); //ok
                fx = sin(MathsBase.mul(MathsBase.sum(m, one), pibyw, MathsBase.sub(X, xmin))); //ok
            }
        }
        Expr fy = null;
        if (maxNorth) {
...
//                fy = cos((n * PI / domain.Height) * (Y - domain.Ymin)); //ok
                fy = cos(MathsBase.mul(n, pibyh, MathsBase.sub(Y, ymin))); //ok
            } else {
//                fy = cos(((2 * n + 1) / 2 * PI / domain.Height) * (Y - domain.Ymin)); //ok
                fy = sin(MathsBase.mul(MathsBase.div(MathsBase.sum(MathsBase.mul(two, n), one), two), pibyh, MathsBase.sub(Y, ymin))); //ok
            }
        } else {
            if (maxSouth) {
                //fy = sin(((2 * n + 1) / 2 * PI / domain.Height) * (Y - domain.Ymin)); //ok
                fy = cos(MathsBase.mul(MathsBase.div(MathsBase.sum(MathsBase.mul(two, n), one), two), pibyh, MathsBase.sub(Y, ymin))); //ok
            } else {
                //fy = sin(((n+1) * PI / domain.Height) * (Y - domain.Ymin)); //ok
                fy = sin(MathsBase.mul(MathsBase.sum(n, one), pibyh, MathsBase.sub(Y, ymin))); //ok
            }
        }
        init(MathsBase.mul(fx, fy, MathsBase.expr(domain)));
    }
