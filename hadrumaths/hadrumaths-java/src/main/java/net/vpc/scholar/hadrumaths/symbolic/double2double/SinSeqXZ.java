 ...
 */
public class SinSeqXZ extends RefDoubleToDouble {
    ...
    private void init(boolean maxNorth, boolean maxEast, boolean maxSouth, boolean maxWest, DoubleParam m, DoubleParam n, Domain domain) {
        ...
        if (maxWest) {
            if (maxEast) {
                ...
                fx = Maths.cos(Maths.mul(two, m, pibyw, Maths.sub(X, xmin))); //ok
            } else {
                //cos((m * 2+1)/2 * pi / width * (X-min))
                fx = Maths.cos(Maths.mul(Maths.div(Maths.sum(Maths.mul(two, m), one), two), pibyw, Maths.sub(X, xmin))); //ok
            }
        } else {
            if (maxEast) {
                //fx = sin(((2 * m + 1) / 2 * PI / domain.width) * (X - domain.xmin)); //ok
                fx = Maths.sin(Maths.mul(Maths.div(Maths.sum(Maths.mul(two, m), one), two), pibyw, Maths.sub(X, xmin))); //ok
            } else {
//                fx = sin(((m+1) * PI / domain.width) * (X - domain.xmin)); //ok
                fx = Maths.sin(Maths.mul(Maths.sum(m, one), pibyw, Maths.sub(X, xmin))); //ok
            }
        }
        Expr fz = null;
        if (maxNorth) {
...
//                fy = cos((n * PI / domain.Height) * (Y - domain.Ymin)); //ok
                fz = Maths.cos(Maths.mul(n, pibyh, Maths.sub(Z, zmin))); //ok
            } else {
//                fy = cos(((2 * n + 1) / 2 * PI / domain.Height) * (Y - domain.Ymin)); //ok
                fz = Maths.cos(Maths.mul(Maths.div(Maths.sum(Maths.mul(two, n), one), two), pibyh, Maths.sub(Z, zmin))); //ok
            }
        } else {
            if (maxSouth) {
                //fy = sin(((2 * n + 1) / 2 * PI / domain.Height) * (Y - domain.Ymin)); //ok
                fz = Maths.sin(Maths.mul(Maths.div(Maths.sum(Maths.mul(two, n), one), two), pibyh, Maths.sub(Z, zmin))); //ok
            } else {
                //fy = sin(((n+1) * PI / domain.Height) * (Y - domain.Ymin)); //ok
                fz = Maths.cos(Maths.mul(Maths.sum(n, one), pibyh, Maths.sub(Z, zmin))); //ok
            }
        }
        exprVal = (Maths.mul(fx, fz, Maths.expr(domain))).toDD();
    }
