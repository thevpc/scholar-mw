 ...
 */
public class VDiscrete extends AbstractDoubleToVector implements Normalizable, PlotHyperCube {
...
//        values[2] = CDiscrete.cst(fx.getDomain(), Complex.ZERO, fx.getDx(), fx.getDy(), fx.getDz(), fx.getAxis()[0], fx.getAxis()[1], fx.getAxis()[2]);
    ...
    public Complex sum() {
        switch (componentDimension.rows) {
            ...
            case 2: {
                return values[0].sum().add(values[1].sum());
            }
            case 3: {
                return values[0].sum().add(values[1].sum()).add(values[2].sum());
            }
        }
        throw new UnsupportedComponentDimensionException(componentDimension.rows);
    }
    ...
    public Complex avg() {
        ...
        switch (componentDimension.rows) {
            ...
            case 2: {
                return values[0].sum().add(values[1].sum()).div(cc);
            }
            case 3: {
                return values[0].sum().add(values[1].sum()).add(values[2].sum()).div(cc);
            }
        }
        throw new UnsupportedComponentDimensionException(componentDimension.rows);
    }
    ...
    public VDiscrete add(VDiscrete other) {
        switch (componentDimension.rows) {
            case 1: {
                return new VDiscrete(
                        values[0].add(other.getComponentDiscrete(Axis.X))
                );
            }
            case 2: {
                return new VDiscrete(
                        values[0].add(other.getComponentDiscrete(Axis.X)),
                        values[1].add(other.getComponentDiscrete(Axis.Y))
                );
            }
            case 3: {
                return new VDiscrete(
                        values[0].add(other.getComponentDiscrete(Axis.X)),
                        values[1].add(other.getComponentDiscrete(Axis.Y)),
                        values[2].add(other.getComponentDiscrete(Axis.Z))
                );
            }
        }
        throw new UnsupportedComponentDimensionException(componentDimension.rows);
...
//    }
    ...
    public Complex integrate(Domain domain, PlaneAxis axis, double fixedValue) {
        ...
        for (int i = 0; i < componentDimension.rows; i++) {
            c.add(getComponentDiscrete(Axis.cartesianValues()[i]).integrate(domain, axis, fixedValue));
        }
        return c.toComplex();
    }
    ...
    public Complex integrate(Domain domain) {
        ...
        MutableComplex c = new MutableComplex();
        for (int i = 0; i < componentDimension.rows; i++) {
            c.add(getComponentDiscrete(Axis.cartesianValues()[i]).integrate(domain));
        }
        return c.toComplex();
    }
    ...
    public PlotCube getCube(int index) {
        CDiscrete i = getComponentDiscrete(Axis.cartesianValues()[index]);
        return new PlotCube(
