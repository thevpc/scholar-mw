...
import net.vpc.scholar.hadrumaths.cache.PersistenceCacheBuilder;
...
import net.vpc.scholar.hadruplot.DefaultPlotModelFactory;
import net.vpc.scholar.hadruplot.PlotDoubleConverter;
import net.vpc.scholar.hadruplot.PlotModelFactory;
import net.vpc.scholar.hadruplot.PlotValueTypeFactory;
...
//    }
    ...
    private static final ExpressionManager defaultExpressionManager = ExpressionManagerFactory.createEvaluator();
...
//    }
    ...
    public static int[] isteps(int min, int max, int step, IntPredicate filter) {
        return net.vpc.common.util.ArrayUtils.isteps(min, max, step, filter);
    }
...
//    }
    ...
    public static <T> Vector<T> sub(Vector<T> a, T b) {
        return a.eval(new VectorOp<T>() {
            @Override
            public T eval(int index, T e) {
                VectorSpace<T> vectorSpace = getVectorSpace(a.getComponentType());
                return vectorSpace.sub(e, b);
            }
        });
    }
...
//    }
    ...
    public static Expr evalExpression(String expression) {
        ExpressionManager m = getExpressionParser();
        Object evaluated = m.createEvaluator(expression).evaluate();
        if (evaluated instanceof Expr) {
            return (Expr) evaluated;
        }
    ...
    public static ExpressionManager getExpressionParser() {
