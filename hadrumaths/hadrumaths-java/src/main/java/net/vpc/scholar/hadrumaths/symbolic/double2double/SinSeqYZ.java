    ...
    private void init(boolean maxNorth, boolean maxEast, boolean maxSouth, boolean maxWest, DoubleParam m, DoubleParam n, Domain domain) {
        ...
        if (maxWest) {
            if (maxEast) {
                ...
                fy = Maths.cos(Maths.mul(two, m, pibyw, Maths.sub(Y, ymin))); //ok
            } else {
                //cos((m * 2+1)/2 * pi / width * (X-min))
                fy = Maths.cos(Maths.mul(Maths.div(Maths.sum(Maths.mul(two, m), one), two), pibyw, Maths.sub(Y, ymin))); //ok
            }
        } else {
            if (maxEast) {
                //fx = sin(((2 * m + 1) / 2 * PI / domain.width) * (X - domain.xmin)); //ok
                fy = Maths.sin(Maths.mul(Maths.div(Maths.sum(Maths.mul(two, m), one), two), pibyw, Maths.sub(Y, ymin))); //ok
            } else {
//                fx = sin(((m+1) * PI / domain.width) * (X - domain.xmin)); //ok
                fy = Maths.sin(Maths.mul(Maths.sum(m, one), pibyw, Maths.sub(Y, ymin))); //ok
            }
        }
        Expr fz = null;
        if (maxNorth) {
...
//                fy = cos(((2 * n + 1) / 2 * PI / domain.Height) * (Y - domain.Ymin)); //ok
        ...
        exprVal = (Maths.mul(fy, fz, Maths.expr(domain))).toDD();
    }
