...
public class EigenvalueDecomposition implements java.io.Serializable {
    private static final long serialVersionUID = -1010101010101001032L;
 ...
 * ------------------------ */
   ...
   //  Fortran subroutine in EISPACK.
      ...
      for (int l = 0; l < n; l++) {
         ...
         tst1 = Maths.max(tst1,Maths.abs(d[l]) + Maths.abs(e[l]));
         int m = l;
         while (m < n) {
   ...
   private void hqr2 () {
      ...
      for (int i = 0; i < nn; i++) {
         ...
         for (int j = Maths.max(i-1,0); j < nn; j++) {
         ...
         // No convergence yet
            ...
            for (int k = m; k <= n-1; k++) {
               ...
               if (s != 0) {
                  ...
                  for (int i = 0; i <= Maths.min(n,k+3); i++) {
                     p = x * H[i][k] + y * H[i][k+1];
                     if (notlast) {
                        p = p + z * H[i][k+2];
                        H[i][k+2] = H[i][k+2] - p * r;
            ...
            for (int i = n-2; i >= 0; i--) {
               ...
               if (e[i] < 0.0) {
                  ...
                  if (e[i] == 0) {
                     ...
                     vi = (d[i] - p) * 2.0 * q;
                     if (vr == 0.0 && vi == 0.0) {
                        vr = eps * norm * (Maths.abs(w) + Maths.abs(q) +
                        Maths.abs(x) + Maths.abs(y) + Maths.abs(z));
                     }
                  ...
                  t = Maths.max(Maths.abs(H[i][n-1]),Maths.abs(H[i][n]));
                  if ((eps * t) * t > 1) {
                     for (int j = i; j <= n; j++) {
                        H[j][n-1] = H[j][n-1] / t;
                        H[j][n] = H[j][n] / t;
      ...
      for (int j = nn-1; j >= low; j--) {
         for (int i = low; i <= high; i++) {
            z = 0.0;
            for (int k = low; k <= Maths.min(j,high); k++) {
               z = z + V[i][k] * H[k][j];
            }
            V[i][j] = z;
         }
