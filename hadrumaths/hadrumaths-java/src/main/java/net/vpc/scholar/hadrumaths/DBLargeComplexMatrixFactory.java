...
//    }
    ...
    public DBLargeComplexMatrixFactory(String id) {
        super();
...
//            ps.close();
    ...
    public ComplexMatrix findMatrix(long id) {
        try {
            ...
            if (rs.next()) {
                return new DBLargeComplexMatrix(id, rs.getInt(1), rs.getInt(2), this);
            }
            rs.close();
            ps.close();
        } catch (SQLException e) {
    ...
    public Long getId(ComplexMatrix m) {
        if (m != null && m instanceof LargeComplexMatrix) {
            return ((LargeComplexMatrix) m).getLargeMatrixId();
        }
        return null;
    }
