...
object CustomFunctions {
  def main(args: Array[String]): Unit = {
    ...
    console.Plot.title("DDXY.composeX").asHeatMap().plot(DDXY.compose(Axis.X,X+Y) * dom)
    console.Plot.title("DDXY.composeXY").asHeatMap().plot(DDXY.compose(Axis.X,X + Y) * dom)
