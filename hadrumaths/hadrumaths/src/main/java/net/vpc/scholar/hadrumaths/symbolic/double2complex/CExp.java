...
public class CExp implements DoubleToComplex {
    ...
    public CExp(Complex amp, double a, double b, Domain domain) {
        ...
        this.real = m.mul(cosAX.mul(cosBY).sub(sinAX.mul(sinBY))).sub(n.mul(cosAX.mul(sinBY).plus(sinAX.mul(cosBY)))).toDD();
        this.imag = m.mul(cosAX.mul(sinBY).plus(sinAX.mul(cosBY))).plus(n.mul(cosAX.mul(cosBY).sub(sinAX.mul(sinBY)))).toDD();
        this.a = a;
        this.b = b;
...
//    }
    ...
    private Expr toCanonical() {
        //amp*exp(iax)*exp(iby)
        return Maths.prod(Maths.exp(Maths.prod(X, î, expr(a))), Maths.exp(Maths.prod(Y, î, expr(b))), amp);
    }
