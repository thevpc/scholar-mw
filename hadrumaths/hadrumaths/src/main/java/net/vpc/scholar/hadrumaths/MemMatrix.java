 ...
 */
public class MemMatrix<T> extends AbstractMatrix<T> implements Serializable {
    private static final long serialVersionUID = 1;

    private VectorSpace<T> vs;
    private T[][] elements;

    MemMatrix(T[][] elements, VectorSpace<T> vs) {
        this.elements = elements;
        this.vs = vs;
    }

    MemMatrix(int rows, int cols, VectorSpace<T> vs) {
        if (rows == 0 || cols == 0) {
    ...
    public Matrix<T> getMatrix(int i0, int i1, int j0, int j1) {
        MemMatrix<T> X = new MemMatrix<T>(i1 - i0 + 1, j1 - j0 + 1, vs);
    ...
    public Matrix<T> getMatrix(int[] r, int[] c) {
        MemMatrix<T> X = new MemMatrix(r.length, c.length, vs);
        T[][] B = X.elements;
        try {
    ...
    public Matrix<T> getMatrix(int r1, int r2, int[] c) {
        MemMatrix<T> X = new MemMatrix(r2 - r1 + 1, c.length, vs);
        T[][] B = X.elements;
        try {
            for (int i = r1; i <= r2; i++) {
                for (int j = 0; j < c.length; j++) {
    ...
    public Matrix<T> getMatrix(int[] r, int c1, int c2) {
        MemMatrix<T> X = new MemMatrix(r.length, c2 - c1 + 1, vs);
        T[][] B = X.elements;
        try {
            for (int i = 0; i < r.length; i++) {
                for (int j = c1; j <= c2; j++) {
    ...
    public Matrix<T> mul(Matrix<T> other) {
        ...
        if (other instanceof MemMatrix) {
            MemMatrix<T> mm = (MemMatrix<T>) other;
    ...
    public Matrix<T> sub(Matrix<T> other) {
        if (other instanceof MemMatrix) {
            ...
            for (int i = 0; i < elements.length; i++) {
                e[i] = newArr(elements[i].length);
                for (int j = 0; j < elements[i].length; j++) {
                    e[i][j] = vs.minus(elements[i][j], mm.elements[i][j]);
                }
            }
            return newMemMatrix(e);
        } else {
            ...
            for (int i = 0; i < elements.length; i++) {
                e[i] = newArr(elements[i].length);
                for (int j = 0; j < elements[i].length; j++) {
                    e[i][j] = vs.minus(elements[i][j], other.get(i, j));
                }
            }
            return newMemMatrix(e);
        }
                        ...
                        if (!vs.isZero(a)) {
                            ...
                            for (int p = 0; p < n; p++) {
                                if (p != k) {
                                    tmp = A[p][l];
                                    for (int q = 0; q < n; q++) {
                                        A[p][q] = vs.minus(A[p][q], vs.mul(A[k][q], vs.div(tmp, a)));
                                        B[p][q] = vs.minus(B[p][q], vs.mul(B[k][q], (vs.div(tmp, a))));
                                    }
                                }
                            }
                        }
...
//            System.out.println("determinant is " + det);
    ...
    @Override
    public MemMatrix<T> coMatrix(int row, int col) {
        int tms = elements.length;
    ...
    public T det() {
        ...
        if (mt instanceof MemMatrix) {
            T[][] matrix = ((MemMatrix<T>) mt).elements;
...
//        System.out.println(new java.util.Date() + " upperTriangle IN (" + primitiveElement3DS.length + ")");
        ...
        T f1;
        T temp;
        int tms = o.elements.length;  // get This Matrix<T> Size (could be smaller than global)
        int v;
