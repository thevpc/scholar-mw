 ...
 */
public final class MemComplexComplexMatrix extends AbstractComplexMatrix implements Serializable {
    private static final long serialVersionUID = 1;

    private Complex[][] elements;

    MemComplexComplexMatrix(Complex[][] elements) {
        this.elements = elements;
    }

    MemComplexComplexMatrix(int rows, int cols) {
        if (rows == 0 || cols == 0) {
    ...
    public ComplexMatrix getMatrix(int i0, int i1, int j0, int j1) {
        MemComplexComplexMatrix X = new MemComplexComplexMatrix(i1 - i0 + 1, j1 - j0 + 1);
    ...
    public ComplexMatrix getMatrix(int[] r, int[] c) {
        MemComplexComplexMatrix X = new MemComplexComplexMatrix(r.length, c.length);
    ...
    public ComplexMatrix getMatrix(int r1, int r2, int[] c) {
        MemComplexComplexMatrix X = new MemComplexComplexMatrix(r2 - r1 + 1, c.length);
        Complex[][] B = X.elements;
        try {
            for (int i = r1; i <= r2; i++) {
                for (int j = 0; j < c.length; j++) {
    ...
    public ComplexMatrix getMatrix(int[] r, int c1, int c2) {
        MemComplexComplexMatrix X = new MemComplexComplexMatrix(r.length, c2 - c1 + 1);
        Complex[][] B = X.elements;
        try {
            for (int i = 0; i < r.length; i++) {
                for (int j = c1; j <= c2; j++) {
    ...
    public ComplexMatrix add(Matrix<Complex> other) {
        if (other instanceof MemComplexComplexMatrix) {
            ...
            for (int i = 0; i < elements.length; i++) {
                e[i] = new Complex[elements[i].length];
                for (int j = 0; j < elements[i].length; j++) {
                    e[i][j] = elements[i][j].plus(mm.elements[i][j]);
                }
            }
            return newMemMatrix(e);
        } else {
            ...
            for (int i = 0; i < elements.length; i++) {
                e[i] = new Complex[elements[i].length];
                for (int j = 0; j < elements[i].length; j++) {
                    e[i][j] = elements[i][j].plus(other.get(i, j));
                }
            }
            return newMemMatrix(e);
        }
    ...
    public ComplexMatrix sub(Matrix<Complex> other) {
        if (other instanceof MemComplexComplexMatrix) {
            ...
            for (int i = 0; i < elements.length; i++) {
                e[i] = new Complex[elements[i].length];
                for (int j = 0; j < elements[i].length; j++) {
                    e[i][j] = elements[i][j].minus(mm.elements[i][j]);
                }
            }
            return newMemMatrix(e);
        } else {
            ...
            for (int i = 0; i < elements.length; i++) {
                e[i] = new Complex[elements[i].length];
                for (int j = 0; j < elements[i].length; j++) {
                    e[i][j] = elements[i][j].minus(other.get(i, j));
                }
            }
            return newMemMatrix(e);
        }
    ...
    public void add(int row, int col, Complex val) {
        elements[row][col] = elements[row][col].plus(val);
    }
    ...
    public void sub(int row, int col, Complex val) {
        elements[row][col] = elements[row][col].minus(val);
    }
...
//            }
    ...
    public ComplexMatrix invBlock(InverseStrategy delegate, int precision) {
        ...
        switch (n) {
            ...
            default: {
                ...
                return getFactory().newMatrix(new ComplexMatrix[][]{
    ...
    @Override
    public MemComplexComplexMatrix invGauss() {
        int m = getRowCount();
                        ...
                        if (!a.isZero()) {
                            ...
                            for (int p = 0; p < n; p++) {
                                if (p != k) {
                                    tmp = A[p][l];
                                    for (int q = 0; q < n; q++) {
                                        A[p][q] = A[p][q].minus(A[k][q].mul(tmp.div(a)));
                                        B[p][q] = B[p][q].minus(B[k][q].mul((tmp.div(a))));
                                    }
                                }
                            }
                        }
     ...
     */
    @Override
    public MemComplexComplexMatrix invAdjoint() {
...
//            System.out.println("determinant is " + det);
    ...
    public Complex det() {
        ...
        ComplexMatrix mt = upperTriangle();
        if (mt instanceof MemComplexComplexMatrix) {
            Complex[][] matrix = ((MemComplexComplexMatrix) mt).elements;
            ...
            for (int row = col + 1; row < tms; row++) {
                ...
                if (!o.elements[col][col].equals(Complex.ZERO)) {
                    ...
                    for (int i = col; i < tms; i++) {
                        o.elements[row][i] = f1.mul(o.elements[col][i]).plus(o.elements[row][i]);
                    }
...
//        System.out.println(new java.util.Date() + " upperTriangle OUT");
    ...
    @Override
    public MemComplexComplexMatrix sparsify(double ceil) {
