...
public abstract class NTxScuffEMComplexNTxSolver extends NTxSolverRunImpl {
    private NTxSweep sweep;
    private NTxSweepTarget sweepParam;

    public NTxScuffEMComplexNTxSolver(String computeName, String solverName, String solverType, NTxSimulationPlan plan) {
    ...
    public List<NTxSimulationResult> execute() {
        ScuffEMStrNTxSimulationPlan plan = (ScuffEMStrNTxSimulationPlan) plan();
        ...
        if (activeSweep != null) {
            ...
            if (dv != null && dv.length > 0) {
                plan.recordRequestedFrequencies(dv);
            }
        }
        plan.ensureSolved();
        ...
        if (plan.rendererContext() != null && plan.rendererContext().compiledDocument() != null) {
    ...
    protected abstract Complex evalComplex(ScuffEMStrNTxSimulationPlan plan, double freq);
}
