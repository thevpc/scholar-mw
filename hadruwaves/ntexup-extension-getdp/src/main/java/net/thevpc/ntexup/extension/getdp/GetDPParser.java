...
public class GetDPParser {

    public static GetDPModelInfo parse(NTxFunctionCallContext args) {
        ...
        for (NTxFunctionArg arg : args.args()) {
            ...
            if (a.isNamedPair()) {
                ...
                switch (key) {
                    ...
                    case "meshresolution":
                    case "resolution": {
                        info.meshResolution = pv.asDoubleValue().orElse(0.5);
                        break;
                    }
                    case "dockerimage":
                    case "image": {
        ...
        if (info.geometryId != null) {
            ...
            if (scene3D != null && Objects.equals(scene3D.type(), "scene3d")) {
                ...
                context.log(NMsg.ofC("GetDP: 'geometry' %s could not be resolved in the current scope", finalGeometryId).asError());
            }
        }
