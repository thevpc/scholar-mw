...
object d_15_12_2017_relativeError_SRCTestFunctions2 {
...
//      var patchtest = vector(essaiPatchX, essaiPatchY)
      ...
      Plot.asCurve().title("gp:"+PLine).asCurve().plot(gp)
      Plot.asCurve().title("<E0,gp>:"+PLine).asHeatMap().plot(Ev)
      Plot.asCurve().title("E0:"+PLine).plot(Eappr)
