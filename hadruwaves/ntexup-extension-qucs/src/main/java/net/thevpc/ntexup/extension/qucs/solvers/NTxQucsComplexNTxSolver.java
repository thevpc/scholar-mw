...
public abstract class NTxQucsComplexNTxSolver extends NTxSolverRunImpl {
    private NTxSweep sweep;
    private NTxSweepTarget sweepParam;

    public NTxQucsComplexNTxSolver(String computeName, String solverName, String solverType, NTxSimulationPlan plan) {
    ...
    public List<NTxSimulationResult> execute() {
        QucsStrNTxSimulationPlan plan = (QucsStrNTxSimulationPlan) plan();
        ...
        if (activeSweep != null) {
            double fmin = activeSweep.rangeFrom != null ? activeSweep.rangeFrom.doubleValue() : 3.0e9;
            double fmax = activeSweep.rangeTo != null ? activeSweep.rangeTo.doubleValue() : 4.4e9;
            int count = activeSweep.count > 0 ? activeSweep.count : 29;
            plan.ensureQucsSolved(fmin, fmax, count);
            ...
            if (dv == null || dv.length == 0) {
                dv = new double[]{activeSweep.rangeFrom != null ? activeSweep.rangeFrom.doubleValue() : (plan.modelInfo != null ? plan.modelInfo.frequency : 3.4e9)};
            }
            for (double fr : dv) {
            ...
            plan.ensureQucsSolved(3.0e9, 4.4e9, 29);
            plan.rendererContext().log(NMsg.ofC("------------------"));
            plan.rendererContext().log(NMsg.ofC("[%s] %s (single value): ", outputName(), solverName()));
            plan.rendererContext().log(NMsg.ofC("------------------"));
            double fr = plan.modelInfo != null && plan.modelInfo.frequency > 0 ? plan.modelInfo.frequency : 3.4e9;
            Complex c = evalComplex(plan, fr);
            complexValues.add(
    ...
    protected abstract Complex evalComplex(QucsStrNTxSimulationPlan plan, double freq);
}
