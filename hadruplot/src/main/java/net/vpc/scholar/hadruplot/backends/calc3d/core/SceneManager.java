...
import net.vpc.scholar.hadruplot.backends.calc3d.elements.*;
...
public class SceneManager {
    ...
    /**
     * bounding box of scene, all primitiveElement3D are clipped against this box
     */
    private Box3D box;
    /**
     ...
     */
    private boolean gridXYVisible = true;
    /**
    ...
    /**
     * Adds Element3D to list of Element3D and also creates its renderable primitiveElement3D
     *
     * @param element3D
     */
     ...
     *
     * @param reCreateEachElement Setting this true forces each Element3D to reCreate its renderable primitiveElement3D
     * @return
     */
    public Scene3D createScene(boolean reCreateEachElement) {
        ...
        if (axisVisible) for (PrimitiveElement3D e : axis3D.primitiveElement3DS) object3D.addElement(e);

        //if (boxVisible)		for (PrimitiveElement3D e:box3D.primitiveElement3DS)	object3D.addElement(e);

        if (gridXYVisible) for (PrimitiveElement3D e : gridXY.primitiveElement3DS) object3D.addElement(e);
        ...
        for (Element3D element3D : element3Ds) {
            ...
            if (element3D instanceof Element3DPlane || element3D instanceof Element3DPolygon) {
                ...
                if ((null != e) && element3D.prefs().isVisible()) object3D.addElement(e);
            }
        }
        ...
        for (Element3D element3D : element3Ds) {
            ...
            PrimitiveElement3D e = reCreateEachElement ? element3D.createElement(clip) : element3D.getPrimitiveElement3D();
            if ((e == null) && !reCreateEachElement) e = element3D.createElement(clip);
            if ((e == null) || !element3D.prefs().isVisible()) continue;
            if (e instanceof PrimitiveElement3DCollection) {
                for (PrimitiveElement3D e1 : ((PrimitiveElement3DCollection) e).primitiveElement3DS) if (null != e1) object3D.addElement(e1);
            } else {
    ...
    private PrimitiveElement3DCollection createXYGrid(Color color) {
        ...
        for (y = minY + dy; y < maxY; y += dy) {
            ...
            if (getSettings().mapCliptoY(y) == 0) continue;
        ...
        for (x = minX + dx; x < maxX; x += dx) {
            ...
            ec.prefs().setBackColor(color);
            gridXY.addElement(ec);
            if (getSettings().mapCliptoY(x) == 0) continue;
