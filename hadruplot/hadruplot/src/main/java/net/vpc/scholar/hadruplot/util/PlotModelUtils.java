...
import net.vpc.scholar.hadruplot.model.value.DefaultPlotValueArrayType;
...
public class PlotModelUtils {
    ...
    public static ValuesPlotModel _defaultPlotModelXY(PlotBuilder builder, String xtitle, String ztitle, double[][] x, Object[][] z, PlotDoubleConverter zDoubleFunction, PlotType plotType) {
        return (ValuesPlotModel)new ValuesPlotModel(builder.getTitle(), xtitle, ztitle, builder.getTitles().toArray(new String[0]),
    ...
    public static ValuesPlotModel _defaultPlotModelXYZ(PlotBuilder builder,
            ...
            double[][] x, double[][] y,
            Object[][] z,
            ToDoubleFunction<Object> zDoubleFunction,
            PlotType plotType, boolean swapxy) {
        if (x == null && builder.getSamples() != null) {
            if (!(builder.getSamples() instanceof AbsolutePlotSamples)) {
                throw new IllegalArgumentException("Samples should be absolute");
            }
            x = new double[][]{((AbsolutePlotSamples) builder.getSamples()).getX()};
        }
        if (y == null && builder.getSamples() != null) {
            if (!(builder.getSamples() instanceof AbsolutePlotSamples)) {
                throw new IllegalArgumentException("Samples should be absolute");
            }
            AbsolutePlotSamples samples = (AbsolutePlotSamples) builder.getSamples();
        ...
        ValuesPlotModel mm = (ValuesPlotModel)new ValuesPlotModel(builder.getTitle(), xtitle, ytitle, ztitle, builder.getTitles().toArray(new String[0]), x, y, z, zDoubleFunction,
    ...
    public static ValuesPlotModel _defaultPlotModelX(PlotBuilder builder,
        ...
        if (x == null && builder.getSamples() != null) {
            ...
            x = ((AbsolutePlotSamples) builder.getSamples()).getX();
        }
        ValuesPlotModel mm = (ValuesPlotModel)new ValuesPlotModel(
    ...
    public static PlotModel _plotComplexArray1(Object[] z, PlotType preferredPlotType, boolean preferColumn, PlotBuilder builder, PlotValueType type) {
        AbsolutePlotSamples aa = null;

        if ((builder.getSamples() instanceof AdaptivePlotSamples)) {
            throw new IllegalArgumentException("Expected Absolute");
        }
        if (builder.getSamples() != null) {
            aa = PlotSamples.toAbsoluteSamples(builder.getSamples(), builder.getDomain());
        }
