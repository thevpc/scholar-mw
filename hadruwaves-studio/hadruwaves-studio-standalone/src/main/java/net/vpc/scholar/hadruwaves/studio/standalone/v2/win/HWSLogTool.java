...
public class HWSLogTool extends JPanel {
    private HadruwavesStudioV2 application;
    private RSyntaxTextArea messagesArea;

    public HWSLogTool(HadruwavesStudioV2 application0) {
        ...
        application.getApplication().log().listeners().add(new PropertyListener() {
            @Override
            public void propertyUpdated(PropertyEvent event) {
                switch (event.getAction()){
                    ...
                    default:{
                        updateAll(application.getApplication().log());
                    }
                }
            }
        });
