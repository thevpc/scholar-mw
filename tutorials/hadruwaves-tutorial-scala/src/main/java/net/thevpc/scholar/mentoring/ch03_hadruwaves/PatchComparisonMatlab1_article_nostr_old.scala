package net.thevpc.scholar.mentoring.ch03_hadruwaves
...
import net.thevpc.scholar.mentoring.extra.EnvConfigurer
...
object PatchComparisonMatlab1_article_nostr_old {
  ...
  var N = 2000 // number of modes TE or TM
  ...
  def build(plot: Boolean): Boolean = {
    ...
    fmnx = fmnxdef.inflate(m.in(0, N - 1).and(n.in(0, N - 1)).and(t.in(0,1)).and(p.in(TE,TM))
