...
object Aguili2_20201008 extends App {
  var ε0 = EPS0
  val εr = 1
  //  val εr =  10.2
  val f0 = 76.064 * GHZ
  ...
  def calcData(f: Double): Data = {
    if (datas.contains(f)) {
      ...
      val nv = new Data(f, f_threshold(f)).find();
  ...
  def plotS_interval_all(): Unit = {
    ...
    Plot.title("d0").
      xsamples(doubles).yformat("metric").xformat("frequency").
      asCurve().
      plot(
        doubles.map(ff => calcData(ff).d0),
        doubles.map(ff => calcData(ff).d),
      )
  }
  ...
  class Data(f: Double, fref: Double) {
    ...
    def find1(): Data = {
      if (f == fs(freqIndexForReferenceD)) {
        ...
        V0 = new DoubleSolverX.Times(G0(_), Vmin, Vmax, Vtimes).solve();
    ...
    def Gρ(ρ: Double): Double = {
      return (1 + sqr(d / w)) * (sqrt(ρ) * π / (1 - ρ)) - f / df
    }
    ...
    def find2(): Data = {
      if (f == fs(freqIndexForReferenceD)) {
        ...
        d = calcData(fs(freqIndexForReferenceD)).d;
        ρ = new DoubleSolverX.Times(Gρ(_), 0.5, 1, Vtimes).solve();
        Ψ = 2 * π * d / λ;
        F = sqrt(ρ) * π / (1 - ρ)
        df2 = f / ((1 + sqr(d / w)) * F)

        d0 = d;
        val Xm = Complex.polar(ρ, Ψ);
        val Ze = Zm * (1 + Xm) / (1 - Xm);

        //        V= (-î * (1+Xm)/(2*Xm) * Zm).toDouble
        V = (-î * (Ze * Zm) / (Ze - Zm)).toDouble
        //        V0 = V;
        //        d0 = d(V);
        //        ρ0 = ρ(V0);
        //        Ψ0 = 2 * π * d0 / λ
