...
import net.vpc.scholar.hadruwaves.{Physics, WallBorders}
import net.vpc.scholar.hadruwaves.mom.{MomStructure, ProjectType, StrLayer}
...
//  Plot.title("Current 4").asHeatMap().domain(plotDomain).plot(J4)
  ...
  Plot.title("str.current().expr()*plotDomain").asHeatMap().domain(plotDomain).plot(JJ*plotDomain)
  Plot.title("str.current().expr()*plotDomain+1E-4*lineDomain").asHeatMap().domain(plotDomain).plot(JJ*plotDomain+1E-4*lineDomain)
  Plot.title("str.current().expr()1E-4*lineDomain").asHeatMap().domain(plotDomain).plot(JJ+1E-4*lineDomain)
  Plot.title("str.current().expr()").asHeatMap().domain(plotDomain).plot(JJ)
}
